...
class HandlerManager(object):
    ...
    if diracSystems['OK']:
      for system in diracSystems['Value']:
        try:
          ...
          services = gConfig.getSections('/Systems/%s/%s/%s' % (system, sysInstance, instance))
          if services['OK']:
            for service in services['Value']:
              ...
              isHTTPS = gConfig.getValue('/Systems/%s/%s/Services/%s/Protocol' % (system, sysInstance, service))
              if isHTTPS and isHTTPS.lower() == 'https':
                self.__service.append(newservice)
  ...
  def loadServicesHandlers(self):
    ...
    if self.__services == True:
      self.discoverHandlers(instance='Services')
  ...
  def loadEndpointsHandlers(self):
    ...
    if self.__endpoints == True:
      self.discoverHandlers(instance='APIs')
    ...
    if self.__endpoints:
      ...
      if not load['OK']:
        return load
      for module in self.loader.getModules().values():
        url = module['classObj'].LOCATION or urlFinder(module['loadName'])
        suffix = '(/[A-z0-9=-_/|]+)?' if module['classObj'].USE_PATH_SUFFIX else ''
        module['classObj'].PATH_RE = re.compile(r'%s%s' % (url, suffix)
