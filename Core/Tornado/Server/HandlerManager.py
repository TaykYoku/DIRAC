...
class HandlerManager(object):
  ...
  def __addHandler(self, handlerTuple, url=None, port=None):
    """
    ...
    if not url or url not in self.__handlers:
      ...
      if url not in self.__handlers:
        self.__handlers[url] = (handlerTuple[1], port)
        ...
        for mName, mObj in inspect.getmembers(handler):
          if inspect.ismethod(mObj) and mName.find(handler.METHOD_PREFIX) == 0:
            ...
            if args:
              url += '[\/]?%s' % '/'.join(args)
            gLogger.debug("  * %s" % url)
            self.__addHandler((module['loadName'], handler), url, ports.get(module['modName']))
    return S_OK()
  ...
  def getHandlersURLs(self):
    ...
    for key in self.__handlers:
      urls.append(TornadoURL(key, self.__handlers[key][0]))
    return urls
